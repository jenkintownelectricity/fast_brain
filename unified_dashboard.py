...
def add_activity(message, icon="", category=None):
    """Add an activity to the log (persisted to database)."""
...
def get_activity():
    ...
    # Fetch from database (already calculates "ago" times)
...
# Load custom prompts from database (replaces in-memory storage)
CUSTOM_PROMPTS = db.get_custom_prompts()
...
def save_golden_prompt(skill_id):
    """Save a custom golden prompt (persisted to database)."""
    ...
    try:
        ...
        # Save to database
        db.save_custom_prompt(skill_id, content)
        ...
        add_activity(f"Updated golden prompt: {skill_id}", "", "prompt")
...
def reset_golden_prompt(skill_id):
    ...
    try:
        ...
        deleted = db.delete_custom_prompt(skill_id)
        # Update in-memory cache
        if skill_id in CUSTOM_PROMPTS:
            del CUSTOM_PROMPTS[skill_id]

        if deleted:
            add_activity(f"Reset golden prompt: {skill_id}", "", "prompt")
